...
const API_KEY = "95be779c9d7f0b585bb8b624e164055b";//process.env.DARKSKY_API_KEY;
...
function getForecast(req, resp) {
  const locationID = req.params.location || '3448433';
  //const url = `${BASE_URL}/data/2.5/group?id=3448433&units=metric&lang=pt_br&appid=${API_KEY}`;
  const url = `${BASE_URL}/data/2.5/onecall?lat=-23.547501&lon=-46.636108&units=metric&lang=pt_br&daily&appid=${API_KEY}`;
  console.log("URL: "+url);
  fetch(url).then((resp) => {
    if (resp.status !== 200) {
    ...
    resp.json(generateFakeForecast(locationID));
  });
}
